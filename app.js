...
app.post('/api/videos/upload', isAuthenticated, videoUpload.array('video', 10), async (req, res) => {
  try {
            ...
            ffmpeg(fullFilePath)
              ...
              .on('end', async () => {
                try {
                  ...
                  console.error('Error preparing video data:', dbError);
                  reject(dbError);
                }
              })
              .on('error', (err) => {
    ...
    if (errors.length === 0) {
      ...
      res.json({
        success: true,
        message: `${results.length} video(s) uploaded successfully, ${errors.length} failed`,
        videos: results.map(r => r.video),
        errors
      });
    }
